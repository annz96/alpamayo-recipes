...
The worker decoder is strictly single-threaded and serial (`video_decode_thread_count=1`, because 80 worker processes per node can't each run their own FFmpeg thread pool). A 6-camera, 4-frame sample nominally needs only 24 target frames (6×4), but actually needs to decode **30-50 frames** of 1080p HEVC — because video is compressed in GOPs (groups of pictures), and to reach a target frame at a given timestamp the decoder has to decode sequentially from that GOP's nearest preceding key frame up to the target frame (it can't just skip ahead), so each target frame usually drags along several non-target frames that have to be decoded with it. A "heavy" sample is one whose target frames sit further from their key frames, or are spread across more GOPs, so it ends up needing noticeably more decoded frames than a typical sample — and a heavy sample's long-tail latency can drag down the whole batch: a worker process delivers samples in FIFO order, so when a heavy sample comes up, it must finish decoding before the next one can be handed off; and since distributed training requires every rank to have a complete batch before the step can start, if even one of the 80 workers is stuck on a heavy sample, every other rank (even ones that were already ready) has to sit and wait — this is exactly the trigger scenario for a severe stall.
